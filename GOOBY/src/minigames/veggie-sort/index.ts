...
type VeggieContext = MinigameContext & {
...
type SortTransition =
  | { kind: "next"; remaining: number; announcement: "reverse" | "normal" | null }
  | { kind: "finish"; remaining: number };
...
export class VeggieSortGame implements MinigameModule {
  ...
  private context: VeggieContext | null = null;
  private root: HTMLElement | null = null;
  private abortController: AbortController | null = null;
  private readonly particleElements = new Map<HTMLElement, number>();
  ...
  private transition: SortTransition | null = null;
  private cardEnteringRemaining = 0;
  ...
  mount(context: MinigameContext): void {
    ...
    const sharedContext = context as VeggieContext;
  ...
  update(deltaSeconds: number): void {
    ...
    if (this.inputLocked > 0) {
      ...
      if (this.inputLocked === 0) {
        this.root.querySelector<HTMLElement>("[data-frenzy]")?.classList.remove("active");
      }
      return;
    }
    if (this.transition) {
      this.transition.remaining = Math.max(0, this.transition.remaining - delta);
      if (this.transition.remaining > 0) return;
      ...
      if (transition.announcement) this.announceRules(transition.announcement);
      this.spawnItem();
      return;
    }
  ...
  private sortCurrent(direction: SortDirection | null, expired = false): void {
    ...
    if (direction) this.actionsTaken += 1;
    ...
    if (result.ended) {
      this.render();
      this.transition = { kind: "finish", remaining: 0.5 };
      return;
    }
    ...
    this.render();
    this.transition = { kind: "next", remaining: 0.33, announcement };
  }
  ...
  private announceRules(mode: "reverse" | "normal"): void {
    ...
    this.emitFeedback("combo", mode === "reverse" ? 5 : 1, "success");
  }
  ...
  private abandonGame(): void {
    ...
    const reward = panel?.querySelector<HTMLElement>("[data-result-reward]");
    const best = panel?.querySelector<HTMLElement>("[data-result-best]");
    if (title) title.textContent = "Shift saved for later";
  ...
  private renderBins(): void {
    if (!this.root) return;
    const assignments = this.state.reverseFrenzy
      ? {
          left: { category: "fruit", emoji: "🍎", label: "FRUIT", accessibleLabel: "fruit" },
          right: { category: "vegetable", emoji: "🥕", label: "VEGGIES", accessibleLabel: "vegetables" },
        }
      : {
          left: { category: "vegetable", emoji: "🥕", label: "VEGGIES", accessibleLabel: "vegetables" },
          right: { category: "fruit", emoji: "🍎", label: "FRUIT", accessibleLabel: "fruit" },
        };
    for (const direction of ["left", "right"] as const) {
      const bin = this.root.querySelector<HTMLButtonElement>(`[data-bin="${direction}"]`);
      if (!bin) continue;
      const assignment = assignments[direction];
      bin.dataset.category = assignment.category;
      bin.setAttribute("aria-label", `Sort ${assignment.accessibleLabel} ${direction}`);
      const emoji = bin.querySelector("span");
      const label = bin.querySelector("b");
      const instruction = bin.querySelector("small");
      if (emoji) emoji.textContent = assignment.emoji;
      if (label) label.textContent = assignment.label;
      if (instruction) instruction.textContent = direction === "left" ? "← SWIPE LEFT" : "SWIPE RIGHT →";
    }
  }
  ...
  private particles(symbol: string, count: number): void {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      this.particleElements.set(particle, 0.75);
    }
  }

  private updateTransientEffects(delta: number): void {
    if (!this.root) return;
    if (this.cardEnteringRemaining > 0) {
      this.cardEnteringRemaining = Math.max(0, this.cardEnteringRemaining - delta);
      if (this.cardEnteringRemaining === 0) {
        this.root.querySelector<HTMLElement>("[data-card]")?.classList.remove("entering");
      }
    }
    if (this.flashRemaining > 0) {
      this.flashRemaining = Math.max(0, this.flashRemaining - delta);
      if (this.flashRemaining === 0) {
        this.root.querySelector<HTMLElement>("[data-flash]")?.classList.remove("active");
      }
    }
    for (const [particle, remaining] of this.particleElements) {
      const next = Math.max(0, remaining - delta);
      if (next > 0) {
        this.particleElements.set(particle, next);
      } else {
        particle.remove();
        this.particleElements.delete(particle);
      }
    }
  }
  ...
  private markup(): string {
    return `
      ...
      <main class="sort-stage">
        ...
        <button class="bin side-bin" data-bin="left" data-category="vegetable" data-direction="left" aria-label="Sort vegetables left"><span>🥕</span><b>VEGGIES</b><small>← SWIPE LEFT</small></button>
        <button class="bin side-bin" data-bin="right" data-category="fruit" data-direction="right" aria-label="Sort fruit right"><span>🍎</span><b>FRUIT</b><small>SWIPE RIGHT →</small></button>
      ...
      <section class="sort-panel" data-panel="tutorial">
        <div class="panel-card">
          ...
          <button class="secondary" data-action="quit">LEAVE THE MARKET</button>
        </div>
      </section>
