/**
 * Firefly Lantern settlement wrapper — one paid settlement per run, unpaid
...
export const FIREFLY_COIN_CAP = 60;
export const FIREFLY_XP_CAP = 140;

/** Pure payout curve: clamped, integer, and never negative. */
export function fireflyPayout(score: number, bestConvoy: number): MinigamePayout {
  const safeScore = Math.max(0, Math.floor(Number.isFinite(score) ? score : 0));
  const safeConvoy = Math.max(0, Math.floor(Number.isFinite(bestConvoy) ? bestConvoy : 0));
  return {
    score: safeScore,
    coins: Math.min(FIREFLY_COIN_CAP, Math.floor(safeScore / 12)),
    xp: Math.min(FIREFLY_XP_CAP, Math.floor(safeScore / 6) + safeConvoy * 4),
  };
}

export interface FireflySettlement {
...
const LOCAL_RUN: MinigameRunId = "firefly-local-run";

export function createFireflySettlement(context: MinigameContext): FireflySettlement {
