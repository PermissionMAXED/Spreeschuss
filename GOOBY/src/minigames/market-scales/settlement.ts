...
export function createMarketSettlement(context: MinigameContext): MarketSettlement {
  ...
  return {
    ...
    begin() {
      settled = false;
      receipt = null;
      runId = context.lifecycle?.beginRun() ?? "market-local-run";
    },
    complete(payout) {
      if (runId === null || settled) return null;
      settled = true;
      if (context.lifecycle && runId !== "market-local-run") {
